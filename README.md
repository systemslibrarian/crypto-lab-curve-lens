...
Elliptic-curve Diffie-Hellman (ECDH) is an asymmetric key-agreement protocol built on the elliptic-curve discrete logarithm problem (ECDLP): given a public point Q = k·G on a curve, recovering the private scalar k is computationally infeasible at production field sizes. This demo implements exact finite-field point addition and scalar multiplication for a small teaching curve (y² = x³ + 2x + 2 mod 17 and analogs), plus real arithmetic for P-256, Curve25519/X25519, secp256k1, and brainpoolP256r1 via @noble/curves. The security model is asymmetric: two parties exchange public points derived from private scalars and independently compute the same shared point without ever transmitting the secret. None of the four production curves shown are post-quantum secure; Shor's algorithm on a sufficiently large fault-tolerant quantum computer solves ECDLP in polynomial time.
...
brainpoolP256r1 (RFC 5639, RFC 7027) is here for a reason beyond being a fourth curve. Its domain parameters are not imported from a library — they are built in this repo from the RFC 5639 §3.4 integers and then validated, before the curve is offered anywhere in the UI, by recomputing the RFC 7027 Appendix A.1 ECDH exchange in both directions and requiring both coordinates of the shared point to match. A curve that fails that check is withheld from the selector rather than shown with a caveat. It also anchors the **seed provenance** note in Panel 4: brainpool's seeds are themselves derived from π and e (RFC 5639 Appendix A.1 and A.2), and the page recomputes both from those constants in your browser and compares them with the published values, next to P-256's seed — which is published, and which P-256 verifiably derives from, but for which no derivation has ever been published.
...
The demo has five interactive panels. Panel 1 (Curve Explorer) starts with a **Reals first (ℝ)** sub-view: slide P and Q along a smooth curve over the real numbers and watch the straight chord hit a third point that reflects down to P+Q — the origin of the group law. Switch to the **Finite field 𝔽₁₇** sub-view to plot every point on the small teaching curve, click any two to compute their exact finite-field sum, and see the same line y ≡ λx + c drawn _wrapped modulo p_ — re-entering the opposite grid edge so it genuinely passes through P, Q, and the third intersection −(P+Q) before the reflection. Panel 2 runs scalar multiplication in both toy and production modes, with a full double-and-add trace for the small field, real generator multiplication for P-256, Curve25519, secp256k1, and brainpoolP256r1, an (x, y)-segmented view of the public point, and a "what changed from the toy curve" note mapping the toy trace to the real one. Panel 3 lets you brute-force the elliptic-curve discrete logarithm on the toy curve — watch G, 2·G, 3·G, … walk the subgroup until the public point appears — then contrasts that with the ~2²⁵⁶ work the same sequential walk would take on P-256, and notes that the best known generic attack, Pollard's rho, needs only ~2¹²⁸ group operations — which is why P-256 is rated at 128-bit security, not 256. Panel 4 shows side-by-side comparison cards for the four production curves (field size, subgroup order, cofactor, SafeCurves rating, post-quantum status) with an inline plain-language glossary for the jargon, plus a **"where did this curve come from?"** note that recomputes brainpoolP256r1's two seeds from π and e and sets them beside NIST P-256's undocumented one. That note is about provenance only: it is not a claim of any weakness in P-256, and no attack on P-256 is known. Panel 5 first shows ECDH on the toy curve as a two-lane diagram — Alice's a·B and Bob's b·A landing on the same highlighted dot, with the colour-coded identity a·(bG) = b·(aG) = (ab)·G — then runs a live real exchange: click "Generate fresh keypairs" to produce new random Alice and Bob key pairs on your chosen curve and verify that both arrive at the same shared value, with one-click copy on every key. The headline value is the raw ECDH output **Z** — the x-coordinate alone, 32 bytes on all four curves (SP 800-56A §5.7.1.2, RFC 5903 §9, RFC 7748 §6.1) — shown beside the full shared point it was taken from, so the value the panel tells you to feed a KDF is the value those specs mean. The private scalars are displayed deliberately and labelled as a teaching view.
...
- [crypto-lab-sleeve-check](https://systemslibrarian.github.io/crypto-lab-sleeve-check/) — the sibling nothing-up-my-sleeve exhibit: GOST's Kuznyechik S-box, published as a bare table, shown to fall out of four small constants and field arithmetic.
...
The finite-field arithmetic, the `@noble/curves` integration, and the rendered UI are all covered by tests. `runVerificationSuite()` additionally re-checks the P-256 and secp256k1 base points and subgroup orders, the RFC 7748 X25519 vectors, and the RFC 7027 Appendix A.1 brainpoolP256r1 exchange at runtime, surfacing the results — including the recomputed brainpool shared point, so it can be read against the RFC — in Panel 2. Spec KATs covered by the unit suite: RFC 7748 (X25519), RFC 7027 Appendix A.1 (brainpoolP256r1, both directions, plus two negative controls), RFC 5639 Appendix A.1/A.2 (all fourteen π- and e-derived seeds), and the ANSI X9.62 / FIPS 186-4 D.3.3 seed-to-curve derivation for P-256. `e2e/claims.spec.ts` additionally asserts that the brainpool shared point rendered on screen equals the RFC 7027 vector. CI (`.github/workflows/ci.yml`) runs lint, tests, and build on every push and pull request; deployment to GitHub Pages runs only after the same checks pass.
