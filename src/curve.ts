...
/**
 * Slope of the group-law line through P and Q (the tangent when P = Q),
 * reduced modulo p. Returns null when the line is vertical (P = -Q or y = 0),
 * in which case the sum is the point at infinity.
 */
export function chordSlope(curve: SmallCurveConfig, left: Point, right: Point): number | null {
  if (curve.kind !== 'weierstrass' || left === null || right === null) {
    return null;
  }
  ...
  if (left.x === right.x && left.y === right.y) {
    ...
    return mod((3 * left.x * left.x + curve.a) * invert(2 * left.y, curve.p), curve.p);
  }

  return mod((right.y - left.y) * invert(right.x - left.x, curve.p), curve.p);
}

export function scalarMultiply(curve: SmallCurveConfig, scalar: number, point: Point): Point {
  if (curve.kind !== 'weierstrass') {
    throw new Error(
      'Scalar multiplication explorer is implemented for the small Weierstrass demo curve.',
    );
  }
...
export function explainScalarMultiply(
...
/**
 * Successive multiples 1·P, 2·P, …, count·P on the small curve.
 * Used to enumerate the cyclic subgroup generated by a point.
 */
export function multiplesOfPoint(
  curve: SmallCurveConfig,
  point: FinitePoint,
  count: number,
): Point[] {
...
export interface DiscreteLogResult {
  /** The recovered scalar k such that k·base = target, or null if none in range. */
  k: number | null;
  /** Every multiple inspected, in order, so the search can be animated. */
  walk: Point[];
  /** Number of point additions performed before the answer was found. */
  steps: number;
}

/**
 * Brute-force the elliptic-curve discrete logarithm on the toy curve:
 * find k with k·base = target by walking 1·base, 2·base, … This is the
 * exact problem that is computationally infeasible at production field sizes.
 */
export function solveDiscreteLog(
  curve: SmallCurveConfig,
  base: FinitePoint,
  target: Point,
): DiscreteLogResult {
  const order = pointOrder(curve, base);
  const walk: Point[] = [];
  let current: Point = null;
  ...
  for (let k = 1; k <= order; k += 1) {
    ...
    if (pointsEqual(current, target)) {
      return { k, walk, steps: k };
    }
  }

  return { k: null, walk, steps: order };
}
